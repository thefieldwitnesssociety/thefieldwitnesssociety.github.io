...
ARTICLE_004 = '/field-notes/lake-natron/'
ARTICLE_005 = '/field-notes/mount-kilimanjaro/'
...
def document(title, description, route, body, current='', noindex=False, author=None, scripts=()):
    ...
    page_class = 'article' if route in (ARTICLE, ARTICLE_002, ARTICLE_003, ARTICLE_004, ARTICLE_005) else current or ('privacy' if route == '/privacy/' else 'utility')
...
def field_notes():
    notes = [
        ('005', ARTICLE_005, 'Mount Kilimanjaro', 'Tanzania',
         'Five days on the Marangu Route.', '1–5 July 2024',
         'Photography and expedition',
         'A porter crossing the upper slopes of Kilimanjaro with the summit cone beyond.', 1536, 1018),
        ('004', ARTICLE_004, 'Lake Natron', 'Tanzania',
         'Water, salt and the living shore.', '3–6 August 2026',
         'Photography and writing',
         'Flamingos scattered across alternating bands of shallow water and exposed ground at Lake Natron.', 1280, 1600),
    ...
    for index, (number, url, title, place, deck, date, media, alt, width, height) in enumerate(notes):
        ...
        small_width, large_width = (768, 1280) if number == '004' else (960, 1600)
        loading = 'fetchpriority="high"' if index == 0 else 'loading="lazy" decoding="async"'
        image_tag = (f'<img src="/assets/field-note-005/kilimanjaro-cover-v2.jpg" '
                     f'sizes="(max-width:640px) calc(100vw - 78px), (max-width:860px) calc(91vw - 58px), (min-width:1875px) 830px, 45vw" '
                     f'width="{width}" height="{height}" alt="{escape(alt)}" {loading}>') if number == '005' else (
                     f'<img src="{image}-960.webp" srcset="{image}-960.webp {small_width}w, {image}-1600.webp {large_width}w" '
                     f'sizes="(max-width:640px) calc(100vw - 78px), (max-width:860px) calc(91vw - 58px), (min-width:1875px) 830px, 45vw" '
                     f'width="{width}" height="{height}" alt="{escape(alt)}" {loading}>')
...
def build():
    ...
    article_004 = (ROOT/'content/field-note-004.html').read_text(encoding='utf-8')
    (ROOT/'field-notes/lake-natron').mkdir(parents=True,exist_ok=True)
    (ROOT/'field-notes/lake-natron/index.html').write_text(document('Lake Natron · Field Note 004 · The Field Witness Society','Water, salt and the living shore. Lake Natron, Tanzania, documented by Ayshe Rosso and Tommaso Bruno, 3–6 August 2026.',ARTICLE_004,article_004,current='notes',author='The Field Witness Society'),encoding='utf-8')
    article_005 = (ROOT/'content/field-note-005.html').read_text(encoding='utf-8')
    (ROOT/'field-notes/mount-kilimanjaro').mkdir(parents=True,exist_ok=True)
    (ROOT/'field-notes/mount-kilimanjaro/index.html').write_text(document('Mount Kilimanjaro · Field Note 005 · The Field Witness Society','Five days on the Marangu Route, Mount Kilimanjaro, Tanzania, 1–5 July 2024.',ARTICLE_005,article_005,current='notes',author='The Field Witness Society'),encoding='utf-8')
    ...
    entries=''.join(f'<url><loc>{ORIGIN}{p}</loc></url>' for p in ['/', '/field-notes/', '/about/', '/contacts/', ARTICLE, ARTICLE_002, ARTICLE_003, ARTICLE_004, ARTICLE_005])
