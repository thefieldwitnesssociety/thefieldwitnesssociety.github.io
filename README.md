...
- Unverified Univers and Submariner font files have been removed from the published file tree. The CSS can use those faces if they are already installed on a visitor's device, with system-font fallbacks otherwise. EB Garamond remains self-hosted with its OFL licence. The exact supplied logo image is used in both header and footer. Reinstate proprietary web fonts only with the appropriate licence evidence. Existing repository history has not been rewritten; this does not establish licences for past use or the separately supplied PDF/brand originals.
